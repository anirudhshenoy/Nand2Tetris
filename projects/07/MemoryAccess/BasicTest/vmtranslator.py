...
class CodeWriter:
    ...
    def writePushPop2(self, command, segment, index):
        if(segment == 'constant'):
            self.assembly_code.append('@' + index)
            self.assembly_code.append('D=A')
        elif(segment == 'static'):
            self.assembly_code.append('@' + self.filename + str(index))
            self.assembly_code.append('D=M')
        elif(segment == 'temp'):
            self.assembly_code.append('@'+str(int(index)+5))
            self.assembly_code.append('D=M')
        else:
            self.assembly_code.append(
                '@' + self._get_segment_shorthand(segment))
            self.assembly_code.append('D=M')
            self.assembly_code.append('@' + index)
            self.assembly_code.append('D=D+A')
            self.assembly_code.append('A=D')
